...
public class QmsDedalinesExcel extends AbstractXlsxView {
	private static final Logger logger = LoggerFactory.getLogger(QmsDedalinesExcel.class);
	...
	protected void buildExcelDocument(Map<String, Object> modelMap, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		String filename = HttpUtils.uriEncoding("QMS마감.xlsx", StandardCharsets.UTF_8);
		...
		headColNameList.add("마감여부");
		headColNameList.add("연도");
		headColNameList.add("분기");
		headColNameList.add("설명");
		...
		for (Map<String, Object> map : list) {
			...
			row.createCell(c++).setCellValue(Converter.toStr(map.get("ACTIVE")));
			row.createCell(c++).setCellValue(Converter.toStr(map.get("QMS_DELN_YEAR")));
			row.createCell(c++).setCellValue(HttpUtils.restoreXss(Converter.toStr(map.get("QMS_DELN_QUAT"))));
			row.createCell(c++).setCellValue(HttpUtils.restoreXss(Converter.toStr(map.get("QMS_DELN_NM"))));
			row.createCell(c++).setCellValue(HttpUtils.restoreXss(Converter.toStr(map.get("QMS_DELN_DESC"))));
		}
