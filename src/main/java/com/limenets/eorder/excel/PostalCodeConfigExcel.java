...
public class PostalCodeConfigExcel extends AbstractXlsxView {
	private static final Logger logger = LoggerFactory.getLogger(PostalCodeConfigExcel.class);
	...
	protected void buildExcelDocument(Map<String, Object> modelMap, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		String filename = HttpUtils.uriEncoding("우편번호.xlsx", StandardCharsets.UTF_8);
		...
		headColNameList.add("REF");
		headColNameList.add("추가일자");
		...
		for (Map<String, Object> map : list) {
			...
			row.createCell(c++).setCellValue(Converter.toStr(map.get("ADDR")));
			row.createCell(c++).setCellValue(Converter.toStr(map.get("USE_F")));
			row.createCell(c++).setCellValue(Converter.toStr(map.get("PT_TEL")));
			row.createCell(c++).setCellValue(Converter.toStr(map.get("INSERT_DT")));
		}
