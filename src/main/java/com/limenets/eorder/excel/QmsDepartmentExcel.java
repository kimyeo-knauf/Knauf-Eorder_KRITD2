...
public class QmsDepartmentExcel extends AbstractXlsxView {
	private static final Logger logger = LoggerFactory.getLogger(QmsDepartmentExcel.class);
	...
	protected void buildExcelDocument(Map<String, Object> modelMap, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		String filename = HttpUtils.uriEncoding("QMS조직및담당자마스터.xlsx", StandardCharsets.UTF_8);
		...
		headColNameList.add("Active");
		headColNameList.add("Department");
		headColNameList.add("Team");
		headColNameList.add("영업 담당자");
		headColNameList.add("설명1");
		...
		for (Map<String, Object> map : list) {
			...
			row.createCell(c++).setCellValue(Converter.toStr(map.get("USER_NM")));
			row.createCell(c++).setCellValue(HttpUtils.restoreXss(Converter.toStr(map.get("QMS_DEPT_REMARK"))));
			row.createCell(c++).setCellValue(HttpUtils.restoreXss(Converter.toStr(map.get("CREATETIME"))));
		}
