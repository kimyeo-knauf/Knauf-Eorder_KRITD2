...
var ckNameJqGrid = 'admin/board/referenceList/jqGridCookie'; // 페이지별 쿠키명 설정. // ####### 설정 #######
ckNameJqGrid += '/gridList'; // 그리드명별 쿠키명 설정. // ####### 설정 #######

var globalColumnOrderStr = toStr(decodeURIComponent(getCookie(ckNameJqGrid)));
var globalColumnOrder = globalColumnOrderStr.split(',');
var categorySelectedNum = 0;

var defaultColModel = [ //  ####### 설정 #######
	{ name:"NO",  			 label:'NO', 	  width:80,  align:'center', sortable:false, formatter:getRownum },
	{ name:"bd_TYPENM",		label:'유형',	  width:140,  align:'center', sortable:true, index:"bd_TYPE" },
	{ name:"bd_DISPLAYTYPE",label:'출력여부',	  width:140,  align:'center', sortable:true,  formatter:setDisplayTypeName, index:"bd_DISPLAYTYPE" },
	{ name:"bd_FILE", 		 label:'첨부파일',	  width:100, align:'center',   sortable:false, formatter:setIcon },
	{ name:"bd_TITLE",  	 label:'제목', 	  width:615, align:'left',   sortable:true },
	{ name:"user_NM",  		 label:'등록자', 	  width:140,  align:'center',   sortable:false },
	{ name:"bd_INDATE",	 label:'등록일', 	  width:100,  align:'center', sortable:true,  formatter:subStirngInDate, index:"bd_INDATE"   },
	{ name:"bd_MODATE",	 label:'수정일', 	  width:100,  align:'center', sortable:true,  formatter:subStirngMoDate, index:"bd_MODATE"   },
	{ name:"viewPopBtn", 		    label:'상세', 	width:100,   align:'center', sortable:false,     formatter:openViewPop },
	{ name:"bd_LINKUSE", 		    label:'링크 여부', 	width:100,   align:'center', sortable:false, formatter:setDisplayLinkUse },
	{ name:"editAndDelBtn", 		    label:'기능', 	width:140,   align:'center', sortable:false,     formatter:setEditBtn },
	{ name:"view_CNT", 		 label:'조회수',	  width:70, align:'right',   sortable:false },
...
// 		sortable: true,
		...
		loadComplete : function(data){			
			...
			*/
			...
			var categ2 = data.referenceCategory3List;
			var categ3 = data.referenceCategory4List;
...
function openViewPop(cellval, options, rowObject){		
	return '<button class="btn btn-default btn-xs" onclick="referenceViewPop(this,\''+rowObject.bd_SEQ+'\');" type="button" title="상세">상세</button>';
}

/**
 * 2025-05-14 정보공유 자료실 링크여부 추가 ijy
 */
function setDisplayLinkUse(cellval, options, rowObject){
	var html = '';
	var linkUse = rowObject.bd_LINKUSE;
	if(linkUse != null && linkUse == 'Y'){
		html = '<button class="btn btn-default btn-xs" onclick="openLink(\''+rowObject.bd_LINK+'\');" type="button" title="링크">링크</button>';
	}
	return html;
}
...
function setEditBtn(cellval, options, rowObject){	
	if(isWriteAuth){
		return '<button onclick="referenceAddEditPop(this,\''+rowObject.bd_SEQ+'\');" type="button" class="btn btn-default btn-xs">수정</button>'
		+ '<button onclick="delReference(this,\''+rowObject.bd_SEQ+'\');" type="button" class="btn btn-default btn-xs">삭제</button>';
	}else{
		return '';
	}
}
 ...
 * 자료실 삭제
 */
function delReference(obj,bdSeq){	
	...
	if(confirm('삭제하시겠습니까?')){
		$.ajax({
			...
			data : params,
			type : 'POST',
			url : '${url}/admin/board/deleteReferenceAjax.lime',
 ...
 * 자료실 등록/수정 폼 팝업
 */
function referenceAddEditPop(obj, bdSeq){		
	$('input[name="r_bdseq"]').val(bdSeq); 	

	// POST 팝업 열기.
	window.open('', 'referenceAddEditPop', setPopupOption());
	
	var $frmPopSubmit = $('form[name="frmPopSubmit"]');
	$frmPopSubmit.attr('action', '${url}/admin/board/reference/pop/addEditPop.lime');
	$frmPopSubmit.attr('method', 'post');
	$frmPopSubmit.attr('target', 'referenceAddEditPop');
	$frmPopSubmit.submit(); 
}
 ...
 * 자료실 한 건 뷰 팝업
 */
function referenceViewPop(obj, bdSeq){		
	$('input[name="r_bdseq"]').val(bdSeq); 		
	// POST 팝업 열기.
	const popup = window.open('', 'referenceViewPop', setPopupOption());
	$frmPopSubmit = $('form[name="frmPopSubmit"]');
	$frmPopSubmit.attr('action', '${url}/admin/board/reference/pop/viewPop.lime');
	$frmPopSubmit.attr('method', 'post');
	$frmPopSubmit.attr('target', 'referenceViewPop');
	$frmPopSubmit.submit();

	popup.onbeforeunload = function() {
		var params = { r_bdseq : bdSeq }
		$.ajax({
			async : true,
			data : params,
			type : 'POST',
			url : './addViewCountAjax.lime',
			success : function( data ){
				dataSearch();
			},
			error : function(request,status,error){
...
function getSearchData(){
	var sData = {
		...
		r_bdtype2 : $('select[name="r_bdtype2"]').val(),
		r_bdtype3 : $('select[name="r_bdtype3"]').val(),
	...
	<main class="page-content content-wrap">
		...
		<div class="page-inner">
			...
			<form name="frmPopSubmit" method="post">
				...
				<input type="hidden" name="r_bdid_pop" value="reference" />
			</form>
			<!-- Main Wrapper -->
			<div id="main-wrapper">
				<!-- Row -->
				<div class="row">
					<div class="col-md-12">
						<div class="panel panel-white">
							...
							<input type="hidden" name="r_bdid" value="reference" />						
								...
								<div class="tableSearch">
									<div class="topSearch">
										<ul>
											...
											<li>
												<label class="search-h">유형 1</label>
												<div class="search-c">
													<select class="form-control" name="r_bdtype" onchange="categorySelected(1)">
											...
											<li>
												<label class="search-h">유형 2</label>
												<div class="search-c">
													<select class="form-control" name="r_bdtype2" onchange="categorySelected(2)">
														<c:forEach items="${referenceCategory3List}" var="list2" varStatus="stat2">
															<c:if test="${stat2.first}">
																<option value="">선택하세요</option>
															</c:if>
															<option value="${list2.CC_CODE}">${list2.CC_NAME}</option>
														</c:forEach>
													</select>
												</div>
											</li>
											...
											<li>
												<label class="search-h">유형 3</label>
												<div class="search-c">
													<select class="form-control" name="r_bdtype3" onchange="categorySelected(3)">
														<c:forEach items="${referenceCategory4List}" var="list3" varStatus="stat3">
															<c:if test="${stat3.first}">
																<option value="">선택하세요</option>
															</c:if>
															<option value="${list3.CC_CODE}">${list3.CC_NAME}</option>
														</c:forEach>
													</select>
												</div>
											</li>
							...
							<div class="panel-body">
								...
								<div class="btnList writeObjectClass">									
									<button class="btn btn-info" onclick="referenceAddEditPop(this, '');" type="button" title="자료실등록">자료실등록</button>
								</div>
