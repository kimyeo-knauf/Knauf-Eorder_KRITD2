...
function dataValidation(){
	...
	var m_bdreply = CKEDITOR.instances.editor1.getData();
	$('input[name="m_bdreply"]').val(m_bdreply);
 ...
 * 자료실 등록/수정
 */
function dataInUp(obj,processType, status){ //processType ADD : 등록 , EDIT : 수정
	...
	$('input[name="m_bdreplyyn"]').val(status);
	var mes = "메세지";
	if(status == 'S')
	{
		mes = "발송";
	...
	if(confirm(mes + '하시겠습니까?'))
		{
		var $frmPop = $('form[name="frmPop"]')		
		$frmPop.attr('action', '${url}/admin/board/insertUpdatesampleAjax.lime');
			...
			<div id="main-wrapper">
				...
				<div class="row">
					...
					<form name="frmPop" method="post" enctype="multipart/form-data">
						<input type="hidden" name="m_bdid" value="sample" />
						...
						<input type="hidden" name="m_bdreplyyn" value="" />
						<div class="panel panel-white">
							<div class="panel-heading clearfix">
								...
								<c:if test="${empty boardOne}">
									<h4 class="panel-title">샘플 등록</h4>
								</c:if>
								<c:if test="${!empty boardOne}">
									<h4 class="panel-title">샘플 관련Comment</h4>
								</c:if>
								...
								<c:if test="${!empty boardOne}">
									<button class="btn btn-info" onclick="dataInUp(this,'REDIT', 'S');" type="button" title="발송">발송</button>
									<button class="btn btn-info" onclick="dataInUp(this,'REDIT', 'R');" type="button" title="반려">반려</button>
									<!--input class="btn btn-info" onclick="dataInUp(this,'REDIT', 'R');" type="button" title="반려">반려</button>
									<input class="favorite styled" type="button" value="Add to favorites">
									<input class="btn btn-info" type="button"   name="r_bdreplyyn"  onclick="dataInUp(this,'REDIT');" value="발송">
									<input class="btn btn-info" type="button"   name="r_bdreplyyn" onclick="dataInUp(this,'REDIT');" value="반려"-->
								</c:if>
								</div>
							</div>
									...
									<div class="topSearch">
										<ul>
											...
											<li>
												...
												<div class="search-c">
													<input type="text" class="search-input form-md-s" name="m_bdtitle" value="${boardOne.BD_TITLE}" onkeyup="checkByte(this, 160);" readonly="readonly" />
...
<%-- 														<c:if test="${boardOne.BD_NOTICEYN eq 'Y' || empty boardOne }"><c:set var='checked' value="checked='checked'"/></c:if> --%>
											...
											<li>
												...
												<div class="search-c">
														<input type="file" class="dropify" name="m_bdfile" value="" <c:if test="${ !empty boardOne.BD_FILE }">data-default-file="${ url }/data/board/${boardOne.BD_FILE}"</c:if> data-max-file-size="300M" data-show-errors="false"/>
...
<%--													<label><input type="radio" name="m_bddisplaytype" value="admin" ${admin} />로그인출력</label>													--%>
							...
							<div class="panel-body">
								...
								<div class="table-responsive">
									<table class="display table dataTable" width="100%" border="0" cellpadding="0" cellspacing="0">
										<tbody>
											<tr>
												<td class="p-v-sm" style='page-break-before: always'>
													${restoreXSSContent}
												</td>
											</tr>
										</tbody>
							...
							<div class="panel-body">
								<h5 class="table-title">수정 *</h5>
								<div class="table-responsive">
									<table class="display table dataTable" width="100%" border="0" cellpadding="0" cellspacing="0">
										<tbody>
											<tr>
												<td class="p-v-sm" style='page-break-before: always'>
													<textarea id="editor1">${boardOne.BD_REPLY}</textarea>
													<input type="hidden" name="m_bdreply" value="" />
												</td>
											</tr>
										</tbody>
									</table>
