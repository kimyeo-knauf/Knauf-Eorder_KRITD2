...
var ckNameJqGrid = 'admin/board/noticeList/jqGridCookie'; // 페이지별 쿠키명 설정. // ####### 설정 #######
...
var defaultColModel = [ //  ####### 설정 #######
	...
	{ name:"bd_FILE", 		 label:'첨부파일',	  width:140, align:'center',   sortable:false, formatter:setIcon },
	{ name:"bd_TITLE",  	 label:'제목', 	  width:755, align:'left',   sortable:true },
...
// console.log('typeof updateComModel = ', typeof updateComModel);
...
function openViewPop(cellval, options, rowObject){
	return '<button class="btn btn-default btn-xs" onclick="noticeViewPop(this,\''+rowObject.bd_SEQ+'\');" type="button" title="상세">상세</button>';
}
...
function setEditBtn(cellval, options, rowObject){
	if(isWriteAuth){
		return '<button class="btn btn-default btn-xs" onclick="noticeAddEditPop(this,\''+rowObject.bd_SEQ+'\');" type="button" title="수정">수정</button>'
		+ '<button class="btn btn-default btn-xs" onclick="delNotice(this,\''+rowObject.bd_SEQ+'\');" type="button" title="삭제">삭제</button>';
	}else{
		return '';
	}
}
...
function delNotice(obj,bdSeq){
	...
	if(confirm('삭제하시겠습니까?')){
		$.ajax({
			...
			data : params,
			type : 'POST',
			url : '${url}/admin/board/deleteNoticeAjax.lime',
 ...
 * 공지사항 등록/수정 폼 팝업
 */
function noticeAddEditPop(obj, bdSeq){
	$('input[name="r_bdseq"]').val(bdSeq);
	// POST 팝업 열기.
	window.open('', 'noticeAddEditPop', setPopupOption());

	var $frmPopSubmit = $('form[name="frmPopSubmit"]');
	$frmPopSubmit.attr('action', '${url}/admin/board/notice/pop/addEditPop.lime');
	$frmPopSubmit.attr('method', 'post');
	$frmPopSubmit.attr('target', 'noticeAddEditPop');
	$frmPopSubmit.submit();
}
 ...
 * 공지사항 한 건 뷰 팝업
 */
function noticeViewPop(obj, bdSeq){
	$('input[name="r_bdseq"]').val(bdSeq);
	// POST 팝업 열기.
	window.open('', 'noticeViewPop', setPopupOption());

	var $frmPopSubmit = $('form[name="frmPopSubmit"]');
	$frmPopSubmit.attr('action', '${url}/admin/board/notice/pop/viewPop.lime');
	$frmPopSubmit.attr('method', 'post');
	$frmPopSubmit.attr('target', 'noticeViewPop');
	$frmPopSubmit.submit();
}
	...
	<main class="page-content content-wrap">
		...
		<div class="page-inner">
			...
			<form name="frmPopSubmit" method="post">
				...
				<input type="hidden" name="r_bdid_pop" value="notice" />
			</form>
			<!-- Main Wrapper -->
			<div id="main-wrapper">
				<!-- Row -->
				<div class="row">
					<div class="col-md-12">
						<div class="panel panel-white">
							...
							<input type="hidden" name="r_bdid" value="notice" />
										...
										<ul>
											...
											<li id="displaytypeTagId">
												...
												<div class="search-c radio">
													...
													<c:set var="loginChk" value="" />
													<c:if test="${param.r_bddisplaytype eq 'user'}"><c:set var="userChk" value="checked" /></c:if>
													<c:if test="${param.r_bddisplaytype eq 'login'}"><c:set var="loginChk" value="checked" /></c:if>

													<label><input type="radio" name="r_bddisplaytype" value="user" ${userChk} onchange="dataSearch()"/>거래처출력</label>
													<label><input type="radio" name="r_bddisplaytype" value="login" ${loginChk} onchange="dataSearch()"/>로그인출력</label>
												</div>
											</li>
											<li>
											...
											<li>
												...
												<div class="search-c radio">
													...
													<label><input type="radio" name="r_bdfileyn" value="Y" ${bdfileynChk} onchange="dataSearch()""/>Y</label>
												</div>
											</li>
										</ul>
									</div>
							...
							<div class="panel-body">
								...
								<div class="btnList writeObjectClass">
									<button class="btn btn-info" onclick="noticeAddEditPop(this, '');" type="button" title="공지사항등록">공지사항등록</button>
								</div>
