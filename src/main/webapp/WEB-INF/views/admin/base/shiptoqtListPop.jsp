...
$(function(){
	$('input[name="rl_quoteqt"]').val(toStr('${param.rl_quoteqt}'));
	getGridListPop();
});
...
/* ↓↓↓↓↓↓↓↓↓ 2025-04-15 hsg. Nuclear lunch Detected : 납품처의 유효기간이 표기 및 유효기간만료된 현장은 선택이 되지 않게 수정 요청 => 납품처의 유효기간이 표기 ↓↓↓↓↓↓↓↓↓ */
...
function dataSelect(obj, rowId, div) { // rowId : 빈값=다중선택, !빈값=개별선택 / div : 1=선택후계속,2=선택후 닫기.
	...
	if('' == rowId){ // 다중.
		...
		// ↓↓↓↓↓↓↓↓↓↓↓ 2025-04-15 hsg. Nuclear lunch Detected : 납품처의 유효기간이 표기 및 유효기간만료된 현장은 선택이 되지 않게 수정 요청 => 납품처 선택시 오늘 날짜와 비교하여 만료날짜가 도래 하였을 경우 선택 시, 선택 불가 메시지 출력 ↓↓↓↓↓↓↓↓↓↓↓ 
		if(!checkBnddt(rowData.BNDDT)){
			alert('만료된 납품처 입니다');
			return;
		}
		// ↑↑↑↑↑↑↑↑↑↑↑ 2025-04-15 hsg. Nuclear lunch Detected : 납품처의 유효기간이 표기 및 유효기간만료된 현장은 선택이 되지 않게 수정 요청 ↑↑↑↑↑↑↑↑↑↑↑
...
function getSearchDataPop(){
	...
	var rl_quoteqt = $('input[name="rl_quoteqt"]').val();
	
	var sData = {
		page_type : page_type
		, r_custcd : r_custcd
		, rl_quoteqt : rl_quoteqt
	};
	return sData;
}

//↓↓↓↓↓↓↓↓↓↓↓ 2025-04-15 hsg. Nuclear lunch Detected : 납품처의 유효기간이 표기 및 유효기간만료된 현장은 선택이 되지 않게 수정 요청 => 오늘 이전의 날짜인 경우 False, 아닌 경우 True ↓↓↓↓↓↓↓↓↓↓↓ 
				...
				<div class="modal-body">
					<div class="tableSearch">
						<div class="topSearch">
							<ul>
								<li>
									<label class="search-h">Quotation QT</label>
									<div class="search-c">
										<input type="text" class="search-input" name="rl_quoteqt" value="${param.rl_quoteqt}" onkeypress="if(event.keyCode == 13){dataSearchPop();}" />
									</div>
								</li>
							</ul>
						</div>
