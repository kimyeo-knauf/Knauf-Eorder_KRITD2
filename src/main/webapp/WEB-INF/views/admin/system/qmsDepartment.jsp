...
var ckNameJqGrid = 'admin/system/qmsDepartment/jqGridCookie'; // 페이지별 쿠키명 설정. // ####### 설정 #######
...
var defaultColModel = [ //  ####### 설정 #######
	...
	{name:"QMS_DEPT_NM"    , label:'Department' , width:300, align:'center', sortable:true, editable:true },
	{name:"QMS_TEAM_NM"    , label:'Team'       , width:180, align:'center', sortable:true, editable:true },
	{name:"USERID"         , label:'담당자ID'   , width:100, align:'center', sortable:true, key:true},
	{name:"QMS_DEPT_ID"    , label:'담당자ID'   , width:100, align:'center', sortable:true, editable:true,hidden:true},
	{name:"USER_NM"        , label:'영업 담당자', width:100, align:'center', sortable:true },
	{name:"QMS_DEPT_REMARK", label:'설명1'      , width:300, align:'left'  , sortable:false, editable:true },
...
function getGridList(){
	...
	$('#gridList').jqGrid({
		url: "${url}/admin/system/getQmsDepartmentListAjax.lime",
...
function dataInUp(obj, val) {
	...
	for (var i=0; i<chkArr.length; i++){
		...
		if(ckflag) ckflag = validation(trObj.find('input[name="QMS_DEPT_NM"]')[0], 'Department', 'value');
		if(ckflag) ckflag = validation(trObj.find('input[name="QMS_TEAM_NM"]')[0], 'Team', 'value');
		...
		// aForm append.
		iFormObj.append('<input type="hidden" name="r_keycode" value="' + toStr(chkArr[i]) + '" />');
		iFormObj.append('<input type="hidden" name="m_userid" value="' + toStr(chkArr[i]) + '" />');
		iFormObj.append('<input type="hidden" name="m_qmsDeptNm" value="' + toStr(trObj.find('input[name="QMS_DEPT_NM"]').val()) + '" />');
		iFormObj.append('<input type="hidden" name="m_qmsTeamNm" value="' + toStr(trObj.find('input[name="QMS_TEAM_NM"]').val()) + '" />');
		iFormObj.append('<input type="hidden" name="m_qmsDeptId" value="' + toStr(trObj.find('input[name="QMS_DEPT_ID"]').val()) + '" />');
		iFormObj.append('<input type="hidden" name="m_qmsDeptRemark" value="' + toStr(trObj.find('input[name="QMS_DEPT_REMARK"]').val()) + '" />');
	...
	if (confirm('저장 하시겠습니까?')) {
		...
		var url = '${url}/admin/system/mergeQmsDepartmentAjax.lime'; 
		$.ajax({
			async : false,
			data : iFormData,
			type : 'POST',
...
function getSearchData(){
	...
	var m_team = $('input[name="m_team"]').val();
	var m_salesrepnm = $('input[name="m_salesrepnm"]').val();
	var m_salesYn  = $('select[name="m_salesYn"]').val();
	var m_activeYn = $('select[name="m_activeYn"]').val();
	var sData = {
		m_dept : m_dept,
		m_team : m_team,
		m_salesrepnm : m_salesrepnm,
		m_salesYn : m_salesYn,
		m_activeYn : m_activeYn
	};
	return sData;
}
...
function excelDown(obj){
	...
	formPostSubmit('frm', '${url}/admin/system/qmsDepartmentExcelDown.lime');
								...
								<div class="tableSearch">
									<div class="topSearch">
										<ul>
											...
											<li>
												<label class="search-h">Team</label>
												<div class="search-c">
													<input type="text" class="search-input" name="m_team" onkeypress="if(event.keyCode == 13){dataSearch();}" />
												</div>
											</li>
											<li>
												<label class="search-h">영업담당자</label>
												<div class="search-c">
													<input type="text" class="search-input" name="m_salesrepnm" onkeypress="if(event.keyCode == 13){dataSearch();}" />
												</div>
											</li>
											<li>
												<label class="search-h">조직 입력여부</label>
												<div class="search-c">
													<select class="form-control" name="m_salesYn" onchange="dataSearch();">
														<option value="Y" selected>입력</option>
														<option value="N">미입력</option>
													</select>
												</div>
											</li>
											<li>
												<label class="search-h">ACTIVE</label>
												<div class="search-c">
													<select class="form-control" name="m_activeYn" onchange="dataSearch();">
