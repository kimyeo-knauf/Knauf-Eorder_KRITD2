...
<script type="text/javascript">
	...
	function dataInUp(obj, processType) { //processType ADD : 등록 , EDIT : 수정
		...
		if (confirm('저장하시겠습니까?')) {
			...
			$frmPop.attr('action', '${url}/front/board/insertUpdatesampleAjax.lime');
		...
		saveRow['shiptoEmail'] = "cs.kr@knauf.com";     //보랄 CS팀 이메일로 바꿔줘야 함
							...
							<div class="panel panel-white">
								<div class="panel-heading clearfix">
									...
									<c:if test="${empty boardOne}">
										<h4 class="panel-title">샘플요청 등록</h4>
									</c:if>
									<c:if test="${!empty boardOne}">
										<h4 class="panel-title">샘플요청 수정</h4>
									</c:if>
									...
									<div class="tableSearch top-view3">
										<div class="topSearch">
											<ul>
												...
												<li><label class="search-h">첨부파일</label>
													<div class="search-c">
														<input type="file" class="dropify" name="m_bdfile"
															value=""
															<c:if test="${ !empty boardOne.BD_FILE }">data-default-file="${ url }/data/board/${boardOne.BD_FILE}"</c:if>
															data-max-file-size="300M" data-show-errors="false" /> <a
															href="${url}/sample/board/boardFileDown.lime?m_bdfile=${boardOne.BD_FILE}"
															class="dropify-file hide m-t-xxxxs m-l-xxs"><i
															class="fa fa-arrow-circle-down pull-left m-t-xxs m-r-xxxs f-s-15"></i>${boardOne.BD_FILE}</a>
														<!-- IE9 -->
													</div></li>
												<%-- li><label class="search-h">출력여부 * </label>
													<div class="search-c radio">
														<c:if
															test="${boardOne.BD_DISPLAYTYPE eq 'user' || empty boardOne }">
															<c:set var='user' value="checked='checked'" />
														</c:if>
														<c:if test="${boardOne.BD_DISPLAYTYPE eq 'admin' }">
															<c:set var='admin' value="checked='checked'" />
														</c:if>
														<!--  label><input type="radio" name="m_bddisplaytype"
															value="user" ${user} />거래처출력</label-->
														<label><input type="radio" name="m_bddisplaytype" value="admin" ${admin} />로그인출력</label>												
													</div></li--%>
												<!-- >li class="blank"><label class="search-h"></label>
