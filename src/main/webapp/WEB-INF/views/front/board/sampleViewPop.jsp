...
function delsample(obj,bdSeq){	
	var params = { r_bdseq : bdSeq , r_bdid : 'sample' }
	console.log(params);
	if(confirm('삭제하시겠습니까?')){
		$.ajax({
			async : false,
			data : params,
			type : 'POST',
			url : '${url}/front/board/deletesampleAjax.lime',
...
/*
function delsample(obj, bdSeq) { //processType ADD : 등록 , EDIT : 수정
	...
	if (confirm('삭제하시겠습니까?')) {
		...
		$frmPop.attr('action',
				'${url}/front/board/deletesampleAjax.lime');
...
<div id="wrap" class="popup-wrapper">
    ...
    <div class="container-fluid product-search">
		...
		<button class="btn btn-info" onclick="delsample(this,${boardOne.BD_SEQ});" type="button" title="저장">삭제</button>
		<button class="btn btn-info" onclick="sampleAddEditPop(this, ${boardOne.BD_SEQ});" type="button" title="sample등록">수정</button>
