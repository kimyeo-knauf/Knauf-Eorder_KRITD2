...
function sendLogin(obj) {
	...
	var params = '';
	params += 'r_userid='+$('#r_userid').val();
	params += '&r_userpwd='+$('#r_userpwd').val();
	params += '&loginToken='+$('#loginToken').val();
	params += '&pageType=F';
	if(isApp()){
		params += '&r_autologinyn='+($('input:checkbox[name="autologinyn"]').is(':checked') ? 'Y' : 'N');
		params += '&where=appajax';
	}
	
	$.ajax({
		async : false,
		//data : {r_userid:$('#r_userid').val(), r_userpwd:$('#r_userpwd').val(), loginToken:$('#loginToken').val(), pageType:'F'},
		data : params,
		type : 'POST',
		url : './loginAjax.lime',
...
<body class="login-wrapper">
			...
			<div class="login-right">
				...
				<div class="login-box">
					...
					<div class="login-footer">
						<p>© 2020 USG BORAL. ALL RIGHTS RESERVED.</p>
						<ul>
	...
	<%--<div  id="" class="layer_greetings"> <!-- 레이어팝업 -->

		<div class="bannerArea">
			<img src="${url}/include/images/front/02.jpg" alt="레이어팝업" />
		</div>
	
		<div class="bottom">
			<input type="checkbox" name="greetings" id="greetings" />
			<label for="greetings"><span>오늘 하루 이 창을 열지 않음</span></label> 
			<a href="javascript:closeGreetings()" title="창닫기"><img src="${url}/include/images/common/pop_close_icon.png" /></a>
		</div>
