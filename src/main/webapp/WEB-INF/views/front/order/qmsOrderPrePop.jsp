...
function getGridList(qmsTempId){ 
	...
	$("#gridList_"+qmsTempId).jqGrid({
		url: "${url}/front/order/getQmsPopPreDetlGridList.lime",
...
function qmsOrderDelete(obj,qmsTempId){
	if (confirm('QMS 사전입력을 정말 취소 하시겠습니까?')) {
		...
		$.ajax({
			...
			type : 'POST',
			url : '${url}/front/order/setQmsFirstOrderCancelAjax.lime',
			success : function(data) {
				alert('취소가 완료되었습니다.');
				if($('#r_reqno').val()){
					opener.parent.location.replace("/eorder/front/order/orderView.lime?r_reqno="+$('#r_reqno').val());
				}else{
					opener.parent.location.replace("/eorder/front/order/orderList.lime");
				}
...
function qmsOrderCancel(obj,qmsTempId){
	//부모페이지 목록으로 이동
	opener.parent.moveOrderList();
	self.close();
}
...
function qmsOrderSave(obj,qmsTempId){
	...
	if (confirm('저장 하시겠습니까?')) {
		...
		$.ajax({
			...
			data : saveRow,
			type : 'POST',
			url : '${url}/front/order/setQmsOrderPreMastUpdate.lime',
			success : function(data) {
				//console.log(data);
			},
			error : function(request,status,error){
		...
		for(var i = 0 ; i < detlRow.length; i++){
			...
			$.ajax({
				...
				data : detlRow[i],
				type : 'POST',
				url : '${url}/front/order/setQmsOrderPreDetlUpdate.lime',
				success : function(data) {
					//console.log(data);
				},
				error : function(request,status,error){
		...
		for(var i = 0 ; i < fireproofChk.length; i++){
			if(i == 0){
				$.ajax({
					...
					data : {'qmsTempId':qmsTempId },
					type : 'POST',
					url : '${url}/front/order/setQmsOrderPreFireproofInit.lime',
					success : function(data) {
						//console.log(data);
					},
					error : function(request,status,error){
			...
			$.ajax({
				...
				type : 'POST',
				url : '${url}/front/order/setQmsOrderPreFireproofUpdate.lime',
				success : function(data) {
					//console.log(data);
				},
				error : function(request,status,error){
...
function openCorpPop(qmsTempId){
	...
	// #POST# 팝업 열기.
	var popUrl = '${url}/front/order/qmsCorpListPop.lime?qmsTempId='+qmsTempId;
		...
		<div class="tab-pane fade" id="tab_${mastlist.QMS_TEMP_ID}">
			...
			<div class="qmsBtnDiv row">
				<c:if test="${admin_authority eq 'QM' or (mastlist.CREATEUSER eq sessionScope.loginDto.userId) }">
					...
					<%-- <button class="btn btn-lg btn-danger"  onclick="qmsOrderDelete(this, '${mastlist.QMS_TEMP_ID}');" type="button" title="삭제">삭제</button> --%>
				</c:if>
				<c:if test="${admin_authority ne 'QM' and (mastlist.CREATEUSER ne sessionScope.loginDto.userId) }">
					<button class="btn btn-lg btn-gray" onclick="qmsOrderCancel(this);" type="button" title="닫기">닫기</button>
				</c:if>
			</div>
		</div>
		</c:forEach>
